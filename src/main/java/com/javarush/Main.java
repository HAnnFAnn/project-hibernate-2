...
public class Main {
    ...
    private Customer createCustomer() {
        try (Session session = sessionFactory.getCurrentSession()) {
            session.beginTransaction();
            Store store = storeDAO.getItems(0,1).get(0);
            City city = cityDAO.getByName("Amroha");

            Address address  = new Address();
            address.setAddress("Moon street, 112");
            address.setCity(city);
            address.setPhone("111-222-333");
            address.setDistrict("disrict12");
            addressDAO.save(address);

            Customer customer = new Customer();
            customer.setActive(true);
            customer.setAddress(address);
            customer.setEmail("aaa@ya.ru");
            customer.setFirstName("Inokentiy");
            customer.setLastName("Barsukov");
